...
if($_GET["action"] == 'fingerPrintlogin')
{
	    ...
	    $Token =  trim($_GET['Token']);  
            ...
            $api_object->query = " SELECT * FROM `login_table`  WHERE `login_table`.`passkey` = '$Token'"; 
            $total_row = $api_object->total_row();
    		...
    		if($total_row == 1)
			{
				...
				foreach($result as $row)
				{
				     ...
				     $data = $api_object->LoginDataFiles($row['username']);
...
if($_GET["action"] == 'FetchGeneralCBTQuestion')
{
    ...
    //https://rsbdigitalshares.com.ng/serverfiles/core_files/school_api.php?action=FetchGeneralCBTQuestion
