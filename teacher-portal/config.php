...
class Loader{
	...
	function query_result()
	{
		...
		return $this->statement->fetchAll();
			...
			function StudentName($online_stu_id)
			{
			...
			function StudentPhoto($online_stu_id)
			{
			...
			function MyUploadedAssessment($school_code,$username)
			{
				 
				 
				$this->query = "SELECT * FROM `50_question_table` WHERE school_code = '$school_code' AND teacher_code = '$username' AND cbt_status ='assessment' ";
			...
			function FecthTecketTeamLead($school_code)
			{
				...
				$this->query ="SELECT `admincode` FROM 1_school_reg   WHERE school_code ='$school_code'";
				$output = $this->query_result();
				foreach($output as $row){ 
					$output =  $row['admincode'];
				} 
			...
			function TeacherSubjectStudentTest($school_code, $subject)
			{ 
			...
			function TeacherSubjectStudentAssesment($school_code, $subject)
			{ 
				//$this->query = "SELECT * FROM `student_test_result` WHERE  `school_code` = '$school_code' AND `school_type` = 'secondary' AND  `$subject` !='' OR `school_code` = '$school_code' AND `$subject` ='null'";
			...
			function FetchSchemeOfWork($school_code,$teacher_code,$subject)
			{
				$this->query = "SELECT * FROM `scheme_of_uploader` WHERE `teacher_id` = '$teacher_code' || `subject_id` = '$subject' AND `school_code` = '$school_code' ";
			...
			function FetchSchemeOfWorkEdit($school_code,$teacher_code,$sow_id)
			{
				$this->query = "SELECT * FROM `scheme_of_uploader` WHERE `school_code` = '$school_code' AND `teacher_id` = '$teacher_code' AND `id` = '$sow_id' ";
