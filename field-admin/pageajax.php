...
if(isset($_POST['page'])){
	...
	if($_POST['page'] == 'admin_signup_page')
	{
		...
		if($_POST['action'] == 'school_signup_action')
	    {
			...
			$loader->query = "SELECT * FROM `1_school_reg` WHERE  `1_school_reg`.`school_code` ='$schoolCode' ";  
			$schoolCode_row_check     = $loader->total_row();
		
		
			$loader->query = "SELECT * FROM `1_school_admins` WHERE  `1_school_admins`.`username` ='$adminhead_email' ";  
			$adminhead_email_row_check     = $loader->total_row();
			...
			if($adminhead_email_row_check == 0)
			{
											...
											$school_logo               = $loader->UploadPhoto($source);  
														...
														$query_wallet =("INSERT INTO 1_school_reg VALUE ( 
															...
															if(mysqli_query($homedb,$query_wallet))
															{
																...
																$loader->send_email($school_email,$adminhead_email, $subject, $body,$schl_head_name);
					...
					else
					{
							...
							$data = array(
								'success'		=>	'failed',
								'feedback'		=>	"School account with $schoolCode already exist"
							);
					}
			}
			else
			{
					...
					$data = array(
						'success'		=>	'failed',
						'feedback'		=>	"School dmin account with $adminhead_email already exist"
					);
			}
